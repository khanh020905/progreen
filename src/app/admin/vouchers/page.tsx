    ...
    <AdminLayout>
      <div className="space-y-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex gap-4">
            <button className="px-6 py-3 bg-white border border-slate-200 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-[2rem] border border-slate-50 shadow-xl shadow-slate-100/50 flex items-center justify-between overflow-hidden relative group">
            ...
            <div className="w-16 h-16 bg-green-50 text-green-600 rounded-[1.5rem] flex items-center justify-center relative z-10">
              <Ticket className="w-8 h-8" />
            </div>
            <div className="absolute top-0 right-0 w-32 h-32 bg-green-50/50 blur-3xl -z-0" />
          </div>
          <div className="bg-white p-8 rounded-[2rem] border border-slate-50 shadow-xl shadow-slate-100/50 flex items-center justify-between overflow-hidden relative group">
            <div className="space-y-4 relative z-10">
          ...
          <div className="p-8 md:p-10 flex flex-col md:flex-row gap-6 border-b border-slate-50 bg-slate-50/30">
            <div className="flex-1 relative">
              ...
              <input 
                ...
                className="w-full pl-14 pr-6 py-4 bg-white rounded-2xl outline-none focus:ring-1 focus:ring-green-500 transition-all font-bold text-sm shadow-sm text-slate-900"
              />
            </div>
          </div>
                  ...
                  <tr key={v._id} className="hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-10 py-6">
                      <div className="flex flex-wrap gap-1">
                        {v.rewards.map((r: any) => (
                          <span key={r._id} className="text-[10px] bg-green-50 text-green-700 px-2.5 py-1 rounded-md font-bold border border-green-100">
                            {r.name}
                          </span>
                        ))}
                      </div>
                    ...
                    <td className="px-10 py-6">
                      {v.isRedeemed ? (
                        ...
                        <span className="text-[10px] text-green-600 font-black uppercase tracking-widest bg-green-50 px-2 py-0.5 rounded">Available</span>
                      )}
                    </td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl max-w-lg w-full p-12 relative overflow-hidden">
            <div className="relative z-10">
              ...
              <form onSubmit={handleCreateVoucher} className="space-y-8">
                <div className="space-y-2">
                  ...
                  <input 
                    type="text" 
                    className="w-full px-8 py-5 bg-slate-50 border-2 border-transparent rounded-2xl text-2xl font-black tracking-[0.2em] uppercase outline-none focus:bg-white focus:border-green-500 transition-all shadow-inner font-mono"
                    ...
                    {rewards.map(r => (
                      <div 
                        ...
                        className={`flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-all ${
                          newVoucher.rewards.includes(r._id) ? 'bg-green-50 border-green-200 border-2' : 'bg-slate-50 border-2 border-transparent hover:bg-slate-100'
                        }`}
                      >
                        <span className="text-sm font-bold text-slate-700">{r.name}</span>
                        {newVoucher.rewards.includes(r._id) && <Check className="w-4 h-4 text-green-600" />}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button type="submit" className="flex-1 py-4 bg-[#0e2114] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-black transition-all">Generate</button>
                </div>
              </form>
            </div>
          </div>
